...
% % idxFail = [1 2 3 4 5 6 7 73 100 107 134 138 150 167];
% mdlName     = 'ICN_model_v15_Mastitskaya2012_control_r2020b';
% for i = 28:40%numPatients
%     parameters = sampleSet(idxStore(i),:);
    numSampleSets = 100;% 100
%     variationFactor = 1.5;
%     mdlAlternative = 4;
%     filename = ['postIR_NADMV_' date '_' num2str(i)];
...
matFilename = ['postIR_NADMV_' date '.mat'];
load(matFilename)
...
% ylabel('t-SNE 2')
% set(gca,'FontSize',16)
% saveas(gcf,['prepostIRaccepted_tsne_NADMV_' date '.png'])

%% Plot parameter distributions for pre-IR and post-IR
% preIR_parameters = sampleSet(idxStore,[42:46]);% ,30:41
% postIR_parameters = paramStore(:,[42:46]);% ,30:41
% parameterNames = {'C_{la} (mL/mm Hg)','P_{0lv} (mm Hg)','k_{Elv} (mm Hg^{-1})','V_{ulv} (mL)','fes_{inf} (Hz)'};
% % {'f_{min,NA}','f_{max,NA}','f_{mid,NA}','k_{NA}',...
% %     'f_{min,NActr}','f_{max,NActr}','f_{mid,NActr}','k_{NActr}',...
% %     'f_{min,DMV}','f_{max,DMV}','f_{mid,DMV}','k_{DMV}'};%,...
%
% saveFilename = 'NADMV_paramDist_031924.png';
...
postIRstore_filename = 'postIR_NADMV_06182024.mat';
postIRbarocurve_filename = 'postIR_NADMV_06182024';
BPvalues = -70:10:90;
brown = [196/255 153/255 124/255];
color = 'm';
filename = ['overlaid_barocurve_NADMV' date '.png'];
