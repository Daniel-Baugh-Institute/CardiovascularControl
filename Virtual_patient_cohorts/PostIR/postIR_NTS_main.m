...
% for i = 33:40%numPatients
%     parameters = sampleSet(idxStore(i),:);
%     numSampleSets = 100;
%     variationFactor = 1.5;
%     mdlAlternative = 3;
%     filename = ['postIR_NTS_' date '_' num2str(i)];
...
matFilename = ['postIR_NTS_' date '.mat'];
% load(matFilename)
...
% ylabel('t-SNE 2')
% set(gca,'FontSize',16)
% saveas(gcf,['prepostIRaccepted_tsne_NTS_' date '.png'])

%% Plot parameter distributions for pre-IR and post-IR
% preIR_parameters = sampleSet(idxStore,[42:46]); % 18:29,
% postIR_parameters = paramStore(:,[42:46]);
% parameterNames = {'f_{min,BR}','f_{max,BR}','f_{mid,BR}','k_{BR}',...
%     'f_{min,CP}','f_{max,CP}','f_{mid,CP}','k_{CP}',...
%     'f_{min,LS}','f_{max,LS}','f_{mid,LS}','k_{LS}',...
%     'C_{la} (mL/mm Hg)','P_{0lv} (mm Hg)','k_{Elv} (mm Hg^{-1})','V_{ulv} (mL)','fes_{inf} (Hz)'};
% saveFilename = 'NTS_paramDist_031924.png';
...
postIRstore_filename = 'postIR_NTS_06232024.mat';
postIRbarocurve_filename ='postIR_NTS_06182024';
BPvalues = -70:10:90;

color = green;
filename = ['overlaid_barocurve_NTS' date '.png'];
