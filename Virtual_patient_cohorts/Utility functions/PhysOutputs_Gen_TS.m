function simRes = PhysOutputs_Gen_TS(simOut,tshort,tlong)
% calculates systolic bloodpressure, diastolic blood pressure, heart rate,
% cardiac output, ejection fractin, and stroke volume for a model given the
% simulink simulation output with values in timeseries format
% Adapted from code from Park 2020
...
% disp('test')
    ...
    Vlv_idx2            = find(diff(Vlv_vals_tshort) <= 0.01 & diff(Vlv_vals_tshort) >= -0.01); % remove duplicate values?
    int_idx2            = intersect(Vlv_idx,Vlv_idx2);
    ESP_idx             = int_idx2(find(diff(int_idx2) > 20)+1);
    timeVec = simOut.time(tshortIdx);
    tt = timeVec(ESP_idx);
%     disp(tt)
    ESP_vals            = Plv_vals_tshort(ESP_idx);
%     plot(timeVec(ESP_idx),ESP_vals)
    ...
    try
        ...
        disp('Warning: CO and EF could not be calculated')
        modelEval.CO = nan;
        modelEval.EjFr = nan;
    end
...
%     modelEval.LSV       = mean(CO_SV.LSV(end-50:end));
    ...
    try
        ...
        disp('Warning: HR is nan')
        simRes(3) = nan;
    end
    simRes(4) = modelEval.CO;
    simRes(5) = modelEval.EjFr;
    simRes(6) = 0;%modelEval.LSV;
    simRes(7) = modelEval.ESP;
    simRes(8) = modelEval.EDP;
