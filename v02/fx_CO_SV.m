...
if length(RSV)  == 1
    ...
    RSV     = diff(RSV);
    if length(RSV) == length(RH_per(2:end-1)) % edited from script provided by james park, which did not have this if statement and skipped directly to else statement, which would produce an NaN if the vectors were not the same length
...
LSV             = cumLV(find(Lres1 >= 1)+3); %cumLV(find(Lres1(:,1) >= 1)+3); edited for constant Tperiod
...
if length(LSV)  == 1
    ...
    if length(LSV) == length(LH_per(2:end-1)) % edited from script provided by james park, which did not have this if statement and skipped directly to else statement, which would produce an NaN if the vectors were not the same length
    ...
    elseif length(LSV) == length(LH_per(2:end))
        ...
        disp('Warning: Left Cardiac Output issue')
        LCO     = NaN;
    end
end
