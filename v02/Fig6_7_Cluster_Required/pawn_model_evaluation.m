function [YY, comp_time] = pawn_model_evaluation(fun_test,XX,varargin)
%
...
% [YY ,compt_time] = pawn_model_evaluation(fun_test,XX)
%
% Input:
...
%
% Output:
% YY = cell array of size (M,n) (number of simulations times number of
%      input variables).  The element X{i,k} is the subsamples
%      of output values to compute the sensitivity of factor i in subsample
%      k, and it is a vector of size (NC,P). NC = number of samples for
%      conditional CDF. P being the number of model outputs associated to
...
else  % added this statement for if there are no additional arguments to evaluate model
    NumExtraArg = 0;
end
